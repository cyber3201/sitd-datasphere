...
import { OM_CONTENT } from '../lib/operatingModelContent';
import { PageContainer } from '../components/PageContainer';
import { SectionCheckbox } from '../components/SectionCheckbox';
import { MarkdownRenderer } from '../components/MarkdownRenderer';

export const OperatingModelLesson: React.FC = () => {
  const { lessonSlug } = useParams<{ lessonSlug: string }>();
  const [isTocOpen, setIsTocOpen] = useState(false);

  // Find current lesson
  const currentIndex = OM_LESSONS.findIndex(l => l.slug === lessonSlug);
  const lessonMeta = OM_LESSONS[currentIndex];
  const content = lessonSlug ? OM_CONTENT[lessonSlug] : undefined;

  // Prev/Next Logic
  const prevLesson = currentIndex > 0 ? OM_LESSONS[currentIndex - 1] : null;
  const nextLesson = currentIndex < OM_LESSONS.length - 1 ? OM_LESSONS[currentIndex + 1] : null;

  if (!lessonMeta || !content) {
    return <Navigate to="/operating-model" replace />;
  }
      ...
      <div className="bg-dg-cream border-b border-dg-blue/10 pt-8 pb-12">
        <PageContainer className="!py-0">
          <div className="flex items-center space-x-2 text-sm text-dg-muted mb-6">
            <Link to="/operating-model" className="hover:text-dg-blue font-medium">Operating Model</Link>
            <span>/</span>
          ...
          <article className="min-w-0">
            ...
            <div className="mt-12 pt-8 border-t border-dg-blue/10">
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-4">
                {prevLesson ? (
                  <Link 
                    to={`/operating-model/${prevLesson.slug}`}
                ...
                {nextLesson ? (
                  <Link 
                    to={`/operating-model/${nextLesson.slug}`}
          ...
          <aside className="hidden lg:block sticky top-32">
            <div className="bg-white border border-dg-blue/10 rounded-xl p-6 shadow-sm">
              ...
              <div className="mt-8 pt-6 border-t border-dg-blue/5">
                <Link to="/operating-model" className="flex items-center text-sm font-semibold text-dg-blue hover:underline">
