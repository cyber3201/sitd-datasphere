
import React from 'react';
import { Link } from 'react-router-dom';
import { 
  ArrowRight, 
  Users, 
  GitMerge, 
  Briefcase, 
  Network,
  Layout,
  Clock,
  Layers,
  FileText,
  DollarSign,
  Calendar
} from 'lucide-react';
import { PageContainer } from '../components/PageContainer';
import { ScrollReveal } from '../components/ScrollReveal';
import { OM_LESSONS } from '../lib/operatingModel';
import { TrackHero } from '../components/TrackHero';

const getIconForTopic = (slug: string) => {
  switch(slug) {
    case 'teams-structure': return <Network size={24} />;
    case 'roles-responsibilities': return <Users size={24} />;
    case 'decision-rights': return <GitMerge size={24} />;
    case 'funding-model': return <DollarSign size={24} />;
    case 'operating-cadence': return <Calendar size={24} />;
    default: return <Users size={24} />;
  }
};

export const OperatingModel: React.FC = () => {
  const scrollToCards = () => {
    const el = document.getElementById('programme');
    if (el) el.scrollIntoView({ behavior: 'smooth' });
  };

  const Illustration = (
    <div className="w-full h-auto aspect-square max-w-[320px] md:max-w-[400px] relative">
       {/* Nodes Graph */}
       <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-3/4 aspect-square">
          {/* Top Node (Central) */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-16 h-16 bg-dg-blue text-white rounded-2xl flex items-center justify-center shadow-lg z-20 border-2 border-white">
             <Users size={24} />
          </div>
          
          {/* Bottom Left (Domain) */}
          <div className="absolute bottom-0 left-0 w-14 h-14 bg-white text-dg-blue rounded-xl flex items-center justify-center shadow-md border border-dg-blue/10 z-10">
             <Briefcase size={20} />
          </div>

          {/* Bottom Right (Domain) */}
          <div className="absolute bottom-0 right-0 w-14 h-14 bg-white text-dg-blue rounded-xl flex items-center justify-center shadow-md border border-dg-blue/10 z-10">
             <Network size={20} />
          </div>

          {/* Connectors */}
          <svg className="absolute inset-0 w-full h-full pointer-events-none overflow-visible">
             <line x1="50%" y1="20%" x2="10%" y2="80%" stroke="#1E3A8A" strokeWidth="2" strokeDasharray="4" opacity="0.3" />
             <line x1="50%" y1="20%" x2="90%" y2="80%" stroke="#1E3A8A" strokeWidth="2" strokeDasharray="4" opacity="0.3" />
             <line x1="10%" y1="80%" x2="90%" y2="80%" stroke="#1E3A8A" strokeWidth="2" strokeDasharray="4" opacity="0.3" />
          </svg>
       </div>
    </div>
  );

  return (
    <div className="bg-dg-cream min-h-screen overflow-x-hidden font-sans">
      
      <TrackHero 
        title="Operating Model"
        subtitle="L’organisation, les processus et la structure des équipes pour une data scalable."
        illustration={Illustration}
        primaryCtaAction={() => window.location.hash = `/operating-model/${OM_LESSONS[0].slug}`}
        secondaryCtaAction={scrollToCards}
      />

      {/* Stats Bar */}
      <div className="relative z-20 -mt-8 sm:-mt-12 md:-mt-16 px-4 mb-12 sm:mb-16">
        <div className="max-w-6xl mx-auto bg-white rounded-2xl shadow-xl md:shadow-2xl shadow-dg-blue/10 border border-dg-blue/5 p-5 md:p-10">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-4 md:gap-8 md:divide-x divide-gray-100">
            <StatItem icon={<Layout className="text-dg-blue" size={24} />} label="Niveau" value="Pro / Mgr" />
            <StatItem icon={<Clock className="text-dg-blue" size={24} />} label="Durée" value="~2.5 Heures" />
            <StatItem icon={<Layers className="text-dg-blue" size={24} />} label="Sujets" value="5 Modules" />
            <StatItem icon={<FileText className="text-dg-blue" size={24} />} label="Format" value="Articles" />
          </div>
        </div>
      </div>

      {/* Cards Grid */}
      <section id="programme" className="pb-20">
        <PageContainer>
          <div className="text-center mb-12">
            <h2 className="text-3xl font-extrabold text-dg-blue mb-4">
              Programme du Cursus
            </h2>
            <p className="text-lg text-dg-muted max-w-2xl mx-auto font-light">
              Modèles centralisés vs fédérés, Data Mesh et Team Topologies.
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {OM_LESSONS.map((lesson, index) => (
              <ScrollReveal key={lesson.id} delay={index * 0.05}>
                <Link to={`/operating-model/${lesson.slug}`} className="block group h-full">
                  <div className="relative h-full bg-gradient-to-br from-[#1E293B] to-[#0F172A] rounded-2xl p-8 overflow-hidden shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 border border-white/5 group-hover:border-dg-mint/20">
                    
                    <div className="absolute inset-0 opacity-10 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] mix-blend-overlay pointer-events-none" />
                    
                    <div className="absolute top-0 right-0 p-6 text-white/10 group-hover:text-dg-mint/20 transition-colors transform group-hover:scale-110 duration-500">
                      {getIconForTopic(lesson.slug)}
                    </div>
                    
                    <div className="relative z-10 flex flex-col h-full justify-between">
                      <div>
                        <div className="flex items-center space-x-3 mb-4">
                          <span className="text-[10px] font-bold text-dg-mint uppercase tracking-widest bg-dg-mint/10 px-2 py-1 rounded border border-dg-mint/20">
                            Article • ~{lesson.estimatedMinutes} min
                          </span>
                        </div>
                        <h3 className="text-2xl font-bold text-white mb-3 group-hover:text-dg-mint transition-colors">
                          {lesson.title}
                        </h3>
                        <p className="text-gray-400 text-sm leading-relaxed mb-6 font-light">
                          {getDescriptionForSlug(lesson.slug)}
                        </p>
                      </div>
                      
                      <div className="flex items-center text-white font-semibold text-sm group/btn">
                        <span>Lire le module</span>
                        <div className="ml-3 w-8 h-8 rounded-full bg-white/10 flex items-center justify-center group-hover:bg-dg-mint group-hover:text-dg-blue transition-all duration-300">
                          <ArrowRight size={16} />
                        </div>
                      </div>
                    </div>
                  </div>
                </Link>
              </ScrollReveal>
            ))}
          </div>
        </PageContainer>
      </section>
    </div>
  );
};

const getDescriptionForSlug = (slug: string) => {
  switch(slug) {
    case 'teams-structure': return "Organisation des équipes : Centralisée, par Domaine (Mesh) ou Platform Team.";
    case 'roles-responsibilities': return "Définition claire des rôles : Product Owner, Data Engineer, Analyst, Steward...";
    case 'decision-rights': return "Qui décide quoi ? Cadre de décision, escalade et résolution de conflits.";
    case 'funding-model': return "Modèles de financement : Chargeback, Showback et budget produit.";
    case 'operating-cadence': return "Les rituels opérationnels : Roadmap, Backlog, Revues et gestion d'incidents.";
    default: return "Module de formation sur le modèle opérationnel data.";
  }
};

const StatItem: React.FC<{ icon: React.ReactNode, label: string, value: string }> = ({ icon, label, value }) => (
  <div className="flex items-center space-x-3 md:space-x-5 md:pl-6 md:first:pl-0">
    <div className="flex-shrink-0 w-12 h-12 md:w-16 md:h-16 bg-dg-blue/5 rounded-2xl flex items-center justify-center">
      {icon}
    </div>
    <div className="min-w-0">
      <p className="text-[10px] md:text-sm text-dg-muted uppercase tracking-wider font-bold mb-0.5 md:mb-1 opacity-80 truncate">{label}</p>
      <p className="text-dg-blue font-bold text-sm md:text-xl leading-tight">{value}</p>
    </div>
  </div>
);
