...
import { DG_CONTENT } from '../lib/dataGovernanceContent';
import { PageContainer } from '../components/PageContainer';
import { SectionCheckbox } from '../components/SectionCheckbox';
import { MarkdownRenderer } from '../components/MarkdownRenderer';

export const DataGovernanceLesson: React.FC = () => {
  const { lessonSlug } = useParams<{ lessonSlug: string }>();
  const [isTocOpen, setIsTocOpen] = useState(false);

  // Find current lesson
  const currentIndex = DG_LESSONS.findIndex(l => l.slug === lessonSlug);
  const lessonMeta = DG_LESSONS[currentIndex];
  const content = lessonSlug ? DG_CONTENT[lessonSlug] : undefined;

  // Prev/Next Logic
  const prevLesson = currentIndex > 0 ? DG_LESSONS[currentIndex - 1] : null;
  const nextLesson = currentIndex < DG_LESSONS.length - 1 ? DG_LESSONS[currentIndex + 1] : null;

  if (!lessonMeta || !content) {
    return <Navigate to="/data-governance" replace />;
  }
      ...
      <div className="bg-dg-cream border-b border-dg-blue/10 pt-8 pb-12">
        <PageContainer className="!py-0">
          <div className="flex items-center space-x-2 text-sm text-dg-muted mb-6">
            <Link to="/data-governance" className="hover:text-dg-blue font-medium">Gouvernance</Link>
            <span>/</span>
          ...
          <article className="min-w-0">
            ...
            <div className="mt-12 pt-8 border-t border-dg-blue/10">
              ...
              <div className="flex flex-col sm:flex-row justify-between gap-4">
                {prevLesson ? (
                  <Link 
                    to={`/data-governance/${prevLesson.slug}`}
                ...
                {nextLesson ? (
                  <Link 
                    to={`/data-governance/${nextLesson.slug}`}
          ...
          <aside className="hidden lg:block sticky top-32">
            <div className="bg-white border border-dg-blue/10 rounded-xl p-6 shadow-sm">
              ...
              <div className="mt-8 pt-6 border-t border-dg-blue/5">
                <Link to="/data-governance" className="flex items-center text-sm font-semibold text-dg-blue hover:underline">
